...
RobotisMotionFile::RobotisMotionFile(string const& filePath)
{
  ...
  if (file == 0)
  {
    cerr << "[RobotisMotionFile::RobotisMotionFile] Can not open motion file: " << filePath << endl;
  ...
  {
    cerr << "[RobotisMotionFile::RobotisMotionFile] Invalid motion file size for " << filePath << " expecting " << expectedSize << " but got " << actualSize << endl;
    fclose(file);
  ...
  for (unsigned pageIndex = 0; pageIndex <= MAX_PAGE_ID; pageIndex++)
  {
    ...
    {
      cerr << "[RobotisMotionFile::RobotisMotionFile] Error seeking file position: " << position << endl;
      fclose(file);
    ...
    {
      cerr << "[RobotisMotionFile::RobotisMotionFile] Error reading page index: " << pageIndex << endl;
      fclose(file);
    ...
    {
      cerr << "[RobotisMotionFile::RobotisMotionFile] Checksum invalid for page index: " << pageIndex << endl;
      fclose(file);
